...
// console.log(factorChain([1, 2, 4, 8, 16, 32])) //➞ tru
...
// Return the sum of all items in an array, where each item is multiplied by its index 
...
function isSpecialArray(arr){
    ...
    let evens = eResult.every(num=>num%2===0)
    let odds = oResult.every(num=>num%2!==0)

    return [evens,odds].every(bool =>bool===true)
...
function reversed(str){
    ...
    // let newStr = ''
    // for(let inital of convert){
    //     if(inital===inital.toLowerCase()){
    //         newStr+=inital.toUpperCase()
    //     } else {
    //         newStr+=inital.toLowerCase()
    //     }
    // }
    // return newStr
...
function censor(str) {
    ...
    //     if(word.length >4){
    //         newStr.push(asterisk.repeat(word.length))
    //     } else {
    //         newStr.push(word)
    //     }
    // }
...
function isIsogram(str){
    converted=str.toLowerCase().split('').sort().join('')
